...
base_dir = os.getcwd().replace('\\', '/')                   # 베이스 디렉토리(VScode 여는 장소)
model_path = "./Models/TestModel/crossValidation.pth"       # 서비스모델 파일경로 
path_rb = '/Web_service/static/logdata/raw_data/*.png'      # 라즈베리파이 촬영이미지 저장소
...
# < 1. img (라즈베리로 촬영한 이미지) >
path_img = '/Web_service/static/result/img' + str(shot) + '.png'
img.save(base_dir + path_img)

# < 2. prediction_for_web (prediction에서 생두종류별 예측된 수) >
path_pred = '/Web_service/static/result/prediction_for_web' + str(shot) + '.pickle'
...
# < 3.img 파일에 레이블링된 이미지 >
path_img_labeled ='/Web_service/static/result/img_labeled' + str(shot) + '.png'
...
for idx, pred in enumerate(prediction):
    if pred[1] == categories[0]:
        ...
        path1 = base_dir + '/Web_service/static/logdata/Normal_pred/'
        ...
        path2 = base_dir + '/Web_service/static/logdata/Broken_pred/'
        ...
        path3 = base_dir + '/Web_service/static/logdata/Black_pred/'
